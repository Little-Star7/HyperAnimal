...
from context_perturbation_noise import generate_perturbed_contexts
...
class DiffusionTrainerLite(LightningLite):
    ...
    def create_and_save_sample_grid(self, model, size, x=None, context=None, latent_decoder=None, save_path=''):
        ...
        with torch.no_grad():
            ...
            # create unconditional samples (or conditional samples with empty contexts)
            ...
            if context is not None:
                ...
                # and fixed synthetic contexts but non-fixed noise conditional generation
                ...
                # and fixed contexts but non-fixed noise conditional generation
    ...
    def run(self, cfg):
        ...
        if cfg.latent_diffusion:
            if cfg.vqvae:
                ...
                latent_encoder = VQEncoderInterface(
                    first_stage_config_path=os.path.join(cfg.paths.root, "models", "autoencoder", "vq_f8_first_stage_config.yaml"),
                    encoder_state_dict_path=os.path.join(cfg.paths.root, "models", "autoencoder", "vq_f8_encoder.pt")
                )
                latent_decoder = VQDecoderInterface(
                    first_stage_config_path=os.path.join(cfg.paths.root, "models", "autoencoder", "vq_f8_first_stage_config.yaml"),
                    decoder_state_dict_path=os.path.join(cfg.paths.root, "models", "autoencoder", "vq_f8_decoder.pt")
                )

            else:
                # create KLGAN encoder and decoder for training in its latent space
                latent_encoder = KLEncoderInterface(
                    first_stage_config_path=os.path.join(cfg.paths.root, "models", "autoencoder",
                                                         "kl_f8_first_stage_config.yaml"),
                    encoder_state_dict_path=os.path.join(cfg.paths.root, "models", "autoencoder", "kl_f8_encoder.pt")
                )
                latent_decoder = KLDecoderInterface(
                    first_stage_config_path=os.path.join(cfg.paths.root, "models", "autoencoder",
                                                         "kl_f8_first_stage_config.yaml"),
                    decoder_state_dict_path=os.path.join(cfg.paths.root, "models", "autoencoder", "kl_f8_decoder.pt")
                )
        ...
        while True:
            for x, context in dataloader:
                ...
                # if the model is not conditioned on contexts
                if context is None or not cfg.model.is_context_conditional:
                    context = None
                    dropout_mask = None
                # else randomly drop some contexts in case of contexts-dropout
                elif cfg.training.context_dropout > 0:
                    dropout_mask = torch.rand(len(x)) < cfg.training.context_dropout
                    # in case of learnable empty contexts embeddings, make sure that at least one contexts is dropped
                ...
                # apply random contexts swaps in case of contexts permutation
                ...
                if cfg.training.id_perturbation > 0:
                    noisy_context = generate_perturbed_contexts(context, min_cos_angle=cfg.training.id_perturbation,
                                                                angle_spread_factor=cfg.training.angle_spread_factor)
                    loss = diffusion_model(x, context=noisy_context, dropout_mask=dropout_mask)
                else:
...
@hydra.main(config_path='configs', config_name='train_syn', version_base=None)
def train(cfg: DictConfig):
    # print(OmegaConf.to_yaml(cfg))
    trainer = DiffusionTrainerLite(devices='1', accelerator='auto', precision=cfg.training.precision)
    trainer.run(cfg)
