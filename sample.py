...
class DiffusionSamplerLite(LightningLite):
    def run(self, cfg) -> Any:
        ...
        # sample size
        size = (train_cfg.constants.image_channels, train_cfg.constants.image_size, train_cfg.constants.image_size)

        if train_cfg.latent_diffusion:
            if train_cfg.vqvae:
                # create VQGAN encoder and decoder for sampling in its latent space
                latent_encoder = VQEncoderInterface(
                    first_stage_config_path=os.path.join(".", "models", "autoencoder",
                                                         "vq_f8_first_stage_config.yaml"),
                ...
                latent_decoder = VQDecoderInterface(
                    first_stage_config_path=os.path.join(".", "models", "autoencoder", "vq_f8_first_stage_config.yaml"),
                    decoder_state_dict_path=os.path.join(".", "models", "autoencoder", "vq_f8_decoder.pt")
                )
                latent_decoder = self.setup(latent_decoder)
                latent_decoder.eval()

            else:
                # create KLGAN encoder and decoder for sampling in its latent space
        ...
        for id_name in context_ids:
            ...
            self.perform_sampling(
                ...
                id_perturbation=cfg.sampling.id_perturbation,
                angle_spread_factor=cfg.sampling.angle_spread_factor
            )
    ...
    def perform_sampling(
            ...
            latent_decoder: torch.nn.Module = None, id_perturbation=0, angle_spread_factor=3):
        ...
        with torch.no_grad():
            for _ in range(n_batches):
                ...
                if id_perturbation > 0:
                    context = generate_perturbed_contexts(context, min_cos_angle=id_perturbation,
                                                          angle_spread_factor=angle_spread_factor)
...
@hydra.main(config_path='configs', config_name='sample_syn', version_base=None)
