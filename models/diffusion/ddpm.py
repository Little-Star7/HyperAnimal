...
class DenoisingDiffusionProbabilisticModel(torch.nn.Module):
    ...
    def forward(self, x0: torch.Tensor, context: torch.Tensor = None,
    ...
    def sample(self, n_samples, size, x_T: torch.Tensor = None, context: torch.Tensor = None,
               dropout_mask: torch.Tensor = None) -> torch.Tensor:
    ...
    def sample_classifier_free_guided(self, n_samples, size, context, guidance_scale,
    ...
    def sample_and_get_step_results(self, n_samples, size, x_T: torch.Tensor = None, context: torch.Tensor = None,
                                    dropout_mask: torch.Tensor = None, result_steps=None) -> torch.Tensor:
        # if initial noise is not provided then sample it
        x_t = x_T if x_T is not None else self.sample_prior(n_samples, size).cuda()
...
#
#     # Define parameters for the DDPM instance
#     T = 10  # Number of diffusion steps
#     criterion = torch.nn.MSELoss()
#     schedule_type = 'linear'
#     schedule_k = 1.0
#     schedule_beta_min = 0.000
#     schedule_beta_max = 0.100
#
#     eps_model = ConditionalUNet()
#
#     # Create an instance of DenoisingDiffusionProbabilisticModel
...
#         schedule_beta_min=schedule_beta_min,
#         schedule_beta_max=schedule_beta_max
#     ).to(device)
#
...
#
#     # Perform a forward pass using the DDPM model
#     loss = ddpm_model(x0)
#
#     # Display the loss
#     print(f"Loss: {loss.item()}")
#
#     # Perform sampling using the DDPM model
#     n_samples = 5
#     size = (channels, height, width)
#     sampled_images = ddpm_model.sample(n_samples=n_samples, size=size)
#
#     # Display sampled images or perform additional testing as needed
#     # ...
#
#     # Perform guided sampling using the DDPM model
#     guided_sampled_images = ddpm_model.sample_classifier_free_guided(
#         n_samples=n_samples,
#         size=size,
#         contexts=torch.randn(batch_size, 512),
#         guidance_scale=0.5
#     )
#
#     # Display guided sampled images or perform additional testing as needed
#     # ...
#
#     # Perform sampling with intermediate results at specific steps
#     result_steps = [0, 5, 10]
#     result_xs = ddpm_model.sample_and_get_step_results(
#         n_samples=n_samples,
#         size=size,
#         result_steps=result_steps
#     )
#
#     # Display intermediate results or perform additional testing as needed
#     print(result_xs)
