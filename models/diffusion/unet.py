...
class ResidualBlock(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, c: torch.Tensor = None):
        ...
        # Adaptive Group Normalization (Dhariwal et al.)
            ...
            # DiffAE uses 'AdaGN' for time condition and this scaling for the contexts (Preechakul et al.)
...
class ConditionalUNet(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, context: torch.Tensor = None, dropout_mask: torch.Tensor = None):
        ...
        # use contexts only if the model is context_conditional
        if self.is_context_conditional:
            if context is None:
                ...
                # if entire samples is dropped out, use the empty contexts embedding instead
                if dropout_mask is not None:
                    c[dropout_mask] = self.empty_context_embedding.type(c.dtype).to(c.device)

                # maybe apply component dropout to counter contexts overfitting
...
if __name__ == "__main__":
    # # Instantiate ResidualBlock
    # residual_block = ResidualBlock(in_channels=64, out_channels=128, time_channels=16, context_channels=32, n_groups=32,
    #                                condition_type='ca', is_context_conditional=True)
    #
    # # Assuming input tensor 'input_tensor', time tensor 'time_tensor', and contexts tensor 'context_tensor'
    # input_tensor = torch.randn(2, 64, 16, 16)
    # time_tensor = torch.randn(2, 16)
    # context_tensor = torch.randn(2, 32)
    # output = residual_block(input_tensor, time_tensor, context_tensor)
    # print(output.shape)


    # Instantiate ConditionalUNet
    conditional_unet = ConditionalUNet(
        ...
        n_context_classes=0,  # Set the appropriate number of contexts classes
        learn_empty_context=False,
        context_dropout_probability=0  # Set the desired dropout probability
    )
    # Assuming time_tensor and context_tensor are 1D tensors
    input_tensor = torch.randn(2, 4, 64, 64)
    time_tensor = torch.randn(2)
    context_tensor = torch.randn(2, 512)

    # Assuming input tensor 'input_tensor', time tensor 'time_tensor', and contexts tensor 'context_tensor'
    output = conditional_unet(input_tensor, time_tensor, context_tensor)
    print(output.shape)
    ...
    #     attention_head_channels=32
    # )
    # x = torch.randn(2, 96, 128, 128)
    # t = torch.randn(2, 384)
    # c = torch.randn(2, 256)
    # output = down_block(x, t, c)
    # print(output.shape)
    ...
    # c = torch.randn(2, 256)
    #
    # # Example usage of Upsample and Downsample modules
    # downsample_module = Downsample(n_channels=96)
    #
    # # Assuming input tensor 'x', time tensor 't', and contexts tensor 'c'
    # output_downsample = downsample_module(x, t, c)
    #
    # # Print the shapes of the output tensors
    # print("Downsample Output Shape:", output_downsample.shape)
