...
from utils.helpers import zero_module, count_model_parameters


class SpatialEmbeddingCrossAttentionBlock(torch.nn.Module):
    """
    Block that applies self-attention to the image input and cross-attention based on an additional contexts embedding
    ...
    def forward(self, x, c):
        ...
        # c is batch of contexts embedding vectors
        ...
        x = rearrange(self.ff(rearrange(self.x_norm(x), "b n d -> b d n").contiguous()),
                      "b d n -> b n d").contiguous() + x
...
if __name__ == "__main__":
    ...
    print(block(x, c).shape)

    # block = SpatialSelfAttentionBlock(in_channels=128, n_heads=4, head_channels=32)
    #
    # trainable_params, _, total_params = count_model_parameters(block)
    # print(f"#Params Model: {trainable_params} (Total: {total_params})")
    #
