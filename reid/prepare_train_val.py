...
split_rate = 0.2  # 将数据集中?%的数据划分到验证集中
img_path = r'/home/moon/yy/reid-animal/stylegan3-data/stylegan3-tiger'  # 数据集存放的地方，建议在程序所在的文件夹下新建一个data文件夹，将需要划分的数据集存放进去
# img_path = r'/home/moon/yy/reid-animal/database/syn/output_rpd'
output_root = r'/home/moon/yy/reid-animal/stylegan3-data/tiger-train'  # 这里是生成的训练集、验证集和测试集所处的位置，这里设置的是在当前文件夹下。
...
# 建立保存训练集的文件夹
train_path = os.path.join(output_root, "train")  # 训练集的文件夹名称为 train
...
# 建立保存验证集的文件夹
val_path = os.path.join(output_root, "val")  # 验证集的文件夹名称为 val
make_file(val_path)
for num_class in data_class:
    # 建立每个类别对应的文件夹
