...
# data
parser.add_argument('--data_dir', default='/home/moon/yy/reid-animal/database/syn_noisy_f1_20', type=str, help='training dir path')
...
if opt.DG:
    image_datasets['DG'] = DGFolder(os.path.join('./redpanda/'),
                                    data_transforms['train'])
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            # Iterate over data.
            for iter, data in tqdm(enumerate(dataloaders[phase]), total=len(dataloaders[phase]), leave=True):
                # get the inputs
                inputs, labels = data
                ...
                if opt.DG and phase == 'train' and epoch > -1:
                    try:
                        _, batch = DGloader_iter.__next__()
                    ...
                    # print("outputs1", outputs1[0].shape, outputs1[1].shape)
                    if return_feature:
                        outputs1, _ = outputs1
                    elif opt.PCB:
                        for i in range(num_part):
...
# if opt.use_dense:
#     model = ft_net_dense(len(class_names), opt.droprate, opt.stride, circle=return_feature, linear_num=opt.linear_num, pre_train=opt.pre_train)
# elif opt.use_NAS:
...
# elif opt.use_swinv2:
#     # model = ft_net_swinv2(len(class_names), (h, w), opt.droprate, opt.stride, circle=return_feature,
#     #                       linear_num=opt.linear_num, pre_train=opt.pre_train)
#     model = ft_net_swinv2(len(class_names), (h, w), opt.droprate, opt.stride, circle=return_feature,
#                           linear_num=opt.linear_num)
#
# elif opt.use_efficient:
#     model = ft_net_efficient(len(class_names), opt.droprate, circle=return_feature, linear_num=opt.linear_num, pre_train=opt.pre_train)
# elif opt.use_hr:
#     model = ft_net_hr(len(class_names), opt.droprate, circle=return_feature, linear_num=opt.linear_num, pre_train=opt.pre_train)
# elif opt.use_convnext:
#     model = ft_net_convnext(len(class_names), opt.droprate, circle=return_feature, linear_num=opt.linear_num, pre_train=opt.pre_train)
# else:
...
if opt.use_dense:
    ...
    model = ft_net_swinv2(len(class_names), (h, w), opt.droprate, opt.stride, circle = return_feature, linear_num=opt.linear_num, pre_train=opt.pre_train)
elif opt.use_efficient:
    model = ft_net_efficient(len(class_names), opt.droprate, circle = return_feature, linear_num=opt.linear_num, pre_train=opt.pre_train)
elif opt.use_hr:
    model = ft_net_hr(len(class_names), opt.droprate, circle = return_feature, linear_num=opt.linear_num, pre_train=opt.pre_train)
    ...
    model = ft_net(len(class_names), opt.droprate, opt.stride, circle = return_feature, ibn=opt.ibn, linear_num=opt.linear_num, pre_train=opt.pre_train)
