...
#
#     def __init__(self, class_num=751, droprate=0.5, stride=2, circle=False, ibn=False, linear_num=512, pre_train=False):
#         super(ft_net, self).__init__()
#         model_ft = models.resnet50(pretrained=True)
#         if pre_train == True:
#             print("pre-train")
#             model_ft.load_state_dict(torch.load("./pre_model/IDPt_resnet50.pth"))
#         if ibn == True:
...
# #
# #     def __init__(self, class_num, input_size=(256, 256), droprate=0.5, stride=2, circle=False, linear_num=512,
# #                  pre_train=False):
...
# #         if pre_train == True:
# #             print("pre-train")
# #             model_ft.load_state_dict(torch.load("./pre_model/IDPt_swinv2.pth"))
# #         self.model = model_ft
# #         self.circle = circle
# #         self.avgpool = nn.AdaptiveAvgPool1d(1)
# #         self.classifier = ClassBlock(1024, class_num, droprate, linear=linear_num, return_f=circle)
...
#
#     def __init__(self, class_num, droprate=0.5, stride=2, circle=False, linear_num=512, pre_train=False):
...
#         if pre_train == True:
#             print("pre-train")
#             model_ft.load_state_dict(torch.load("./pre_model/IDPt_convnext.pth"))
#         self.model = model_ft
...
#     def __init__(self, class_num, droprate=0.5, circle=False, linear_num=512, pre_train=False):
#         super().__init__()
...
#         if pre_train == True:
#             print("pre-train")
#             model_ft.load_state_dict(torch.load("./pre_model/IDPt_hr.pth"))
#         self.model = model_ft
#         self.circle = circle
#         self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
#         self.classifier = ClassBlock(2048, class_num, droprate, linear=linear_num, return_f=circle)
...
#         if pre_train == True:
#             print("pre-train")
#             model_ft.load_state_dict(torch.load("./pre_model/IDPt_dense.pth"))
#         self.model = model_ft
#         self.circle = circle
...
#         if pre_train == True:
#             print("pre-train")
#             model_ft.load_state_dict(torch.load("./pre_model/IDPt_efficient.pth"))
...
class ft_net(nn.Module):
    ...
    def __init__(self, class_num=751, droprate=0.5, stride=2, circle=False, ibn=False, linear_num=512, pre_train=False):
        ...
        if pre_train:
            print("pre-train")
            model_ft.load_state_dict(torch.load("./pre_model/gp_IDPt_resnet50.pth"))
        if ibn == True:
            model_ft = torch.hub.load('XingangPan/IBN-Net', 'resnet50_ibn_a', pretrained=True)
        # avg pooling to global pooling
        if stride == 1:
...
class ft_net_swinv2(nn.Module):
    ...
    def __init__(self, class_num, input_size=(256, 128), droprate=0.5, stride=2, circle=False, linear_num=512, pre_train=False):
        super(ft_net_swinv2, self).__init__()
        model_ft = timm.create_model('swinv2_base_window8_256', pretrained=False, img_size=input_size,
                                     drop_path_rate=0.2)
        model_full = timm.create_model('swinv2_base_window8_256', pretrained=True)
        ...
        if pre_train:
            print("pre-train")
            model_ft.load_state_dict(torch.load("./pre_model/gp_IDPt_swinv2.pth"))
        self.model = model_ft
        self.circle = circle
        self.avgpool1d = nn.AdaptiveAvgPool1d(1)
        self.avgpool2d = nn.AdaptiveAvgPool2d((1, 1))
...
class ft_net_convnext(nn.Module):
    ...
    def __init__(self, class_num, droprate=0.5, stride=2, circle=False, linear_num=512, pre_train=False):
        ...
        if pre_train:
            print("pre-train")
            model_ft.load_state_dict(torch.load("./pre_model/gp_IDPt_convnext.pth"))
        self.model = model_ft
        # self.model.apply(activate_drop)
        self.circle = circle
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
...
class ft_net_hr(nn.Module):
    def __init__(self, class_num, droprate=0.5, circle=False, linear_num=512, pre_train=False):
        ...
        if pre_train:
            print("pre-train")
            model_ft.load_state_dict(torch.load("./pre_model/gp_IDPt_hr.pth"))
        self.model = model_ft
        self.circle = circle
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.classifier = ClassBlock(2048, class_num, droprate, linear=linear_num, return_f=circle)
...
class ft_net_dense(nn.Module):
    ...
    def __init__(self, class_num, droprate=0.5, stride=2, circle=False, linear_num=512, pre_train=False):
        ...
        if pre_train:
            print("pre-train")
            model_ft.load_state_dict(torch.load("./pre_model/gp_IDPt_dense.pth"))
        self.model = model_ft
        self.circle = circle
        # For DenseNet, the feature dim is 1024
        self.classifier = ClassBlock(1024, class_num, droprate, linear=linear_num, return_f=circle)
...
class ft_net_efficient(nn.Module):
    ...
    def __init__(self, class_num, droprate=0.5, circle=False, linear_num=512, pre_train=False):
        ...
        if pre_train:
            print("pre-train")
            model_ft.load_state_dict(torch.load("./pre_model/gp_IDPt_efficient.pth"))
        model_ft.classifier = nn.Sequential()
        self.model = model_ft
        self.circle = circle
        # For EfficientNet, the feature dim is not fixed
